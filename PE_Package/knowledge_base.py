...
math_problems = [   
    {
        "题目": "求二次函数f(x)=x²−5x+6的零点。",
        "解答": "解题思路：1.理解函数：函数 f(x) = x^2 - 5x + 6 是一个标准的二次函数。2.设定零点条件：零点即 f(x) = 0，所以方程为 x^2 - 5x + 6 = 0。3.因式分解：尝试将方程因式分解为 (x - a)(x - b) = 0 的形式。寻找两个数 a 和 b，使得 a * b = 6 且 a + b = 5。显然，a = 2 和 b = 3 满足条件。写出因式分解形式：(x - 2)(x - 3) = 0。4.求解零点：x - 2 = 0 ⇒ x = 2；x - 3 = 0 ⇒ x = 3。5.最终答案：函数的零点为 x = 2 和 x = 3。"
    },
    {
        "题目": "求二次函数 f(x) = 3x^2 - 7x + 2 的零点。",
        "解答": "解题思路:1.理解函数：函数 f(x) = 3x^2 - 7x + 2 是一个二次函数。2.设定零点条件：零点即 f(x) = 0，所以方程为 3x^2 - 7x + 2 = 0。3.因式分解：尝试将方程因式分解，但发现无法找到满足条件的整数解。4.使用求根公式：对于一般二次方程 ax^2 + bx + c = 0，其根为：x = [-b ± √(b^2 - 4ac)] / (2a)这里 a = 3，b = -7，c = 2。计算判别式：Δ = b^2 - 4ac = 49 - 24 = 25  6.判别式 Δ > 0，说明方程有两个不同的实数根。求解零点：x = [7 ± √25] / 6 = [7 ± 5] / 6,x = (7 + 5) / 6 = 12 / 6 = 2,x = (7 - 5) / 6 = 2 / 6 ≈ 0.333。7.验证：将x=2和x=0.333分别代回原方程，函数值=0。结论：函数 f(x) = 3x^2 - 7x + 2 的零点为 x = 2 和 x ≈ 0.333。8.最终答案：函数的零点为 x = 2 和 x ≈ 0.333。"
    },
    {
        "题目": "求二次函数 f(x) = x^2 + 4x + 5 的零点。",
        "解答": "解题思路：1.理解函数：函数 f(x) = x^2 + 4x + 5 是一个二次函数。2.设定零点条件：零点即 f(x) = 0，所以方程为 x^2 + 4x + 5 = 0。3.因式分解：尝试将方程因式分解，但发现无法找到满足条件的整数解。4.使用求根公式：对于一般二次方程 ax^2 + bx + c = 0，其根为：x = [-b ± √(b^2 - 4ac)] / (2a),这里 a = 1，b = 4，c = 5。5.计算判别式：Δ = b^2 - 4ac = 16 - 20 = -4 6.判别式 Δ < 0，说明方程无实数根。结论：函数 f(x) = x^2 + 4x + 5 无实数零点。7.最终答案:函数无实数零点。"
    },
    {
        "题目": "小明有5个苹果，他给了小红3个，又买了2个回来，问小明现在有多少个苹果？",
        "解答": "小明一开始有5个苹果，给了小红3个后剩下5-3=2个，又买了2个回来，所以现在有2+2=4个苹果。"
    },
    {
        "题目": "已知x + 3 = 7，求x的值。",
        "解答": "方程x + 3 = 7，将3移到等式右边得到x = 7 - 3，所以x = 4。"
    },
    {
        "题目": "一个正方形的边长为4厘米，求它的面积和周长。",
        "解答": "正方形面积=边长×边长=4×4=16平方厘米，周长=4×边长=4×4=16厘米。"
    },
    {
        "题目": "一个袋子里有3个红球和2个蓝球，随机摸出一个球，求摸到红球的概率。",
        "解答": "总共有5个球，红球有3个，所以摸到红球的概率是3/5。"
    },
    {
        "题目": "一辆汽车以每小时60公里的速度行驶，行驶了2.5小时，求它行驶的路程。",
        "解答": "路程=速度×时间=60×2.5=150公里。"
    },
    {
        "题目": "小明、小红、小华三个人中，一个是医生，一个是教师，一个是工程师。已知小明不是教师，小红不是医生，小华是工程师。请问他们各自的职业是什么？",
        "解答": "小华是工程师，小明不是教师所以是医生，小红不是医生所以是教师。"
    },
    {
        "题目": "计算1/2 + 1/3的值。",
        "解答": "找到公分母6，将1/2转换为3/6，1/3转换为2/6，相加得到5/6。"
    },
    {
        "题目": "证明三角形的内角和为180度。",
        "解答": "通过画平行线和利用同位角、内错角的性质，可以证明三角形内角和为180度。"
    },
    {
        "题目": "解方程x² - 5x + 6 = 0。",
        "解答": "因式分解为(x-2)(x-3)=0，解得x=2或x=3。"
    },
    {
        "题目": "某次公司年会上，A、B、C、D四人中有一人获得“年度最佳员工”。结果公布前，四人预测如下：A说：“获奖者是B或C。”B说：“获奖者一定是我。”C说：“获奖者不是我。”D说：“获奖者要么是A，要么是D。”最终结果显示，只有一人的预测正确，其余三人均错误。问：实际获奖者是谁？",
        "解答": """1. 明确预测内容：A、B、C、D四人中有一人获得“年度最佳员工”。A说：“获奖者是B或C。”B说：“获奖者一定是我。”C说：“获奖者不是我。”D说：“获奖者要么是A，要么是D。”
        2. 明确题目条件：最终结果显示，只有一人的预测正确，其余三人均错误。
        3.假设分析，逐一验证：首先，可能的获奖组合是一个人，所以需要考虑所有可能的情况，然后逐一验证他们的预测是否符合条件。
        ##可能的获奖人组合共有4种可能：
        1. A获奖
        2. B获奖
        3. C获奖
        4. D获奖
        ##然后，对每个组合分析每个预测是否正确，再统计正确的数目是否为1。
        让我们逐一分析：
        # 1.被提升的是A：
        A错误，B错误，C正确，D正确（因为A被提升），所以两人正确，不符合。
        # 2.被提升的是B：
        A正确，B正确，C错误，D错误，这样两人正确，不符合。
        # 3.被提升的是C：
        A的预测是B或C，正确；B的预测是自己，错误；C的预测说自己没被提升，错误；D的预测是A或D，错误。这样只有A正确，符合条件。
        题目要求是只有一人正确，所以正确。
        # 4.被提升的是D：
        A错误，B错误，C正确，D正确，两人正确，不符合。
        ##所以目前看，C获奖的情况下，正确的预测是D一人，其余三人错误，符合条件。
        4.正确性验证：被提升的是C的情况下：A正确（因为C被提升），B错误，C错误（因为C被提升，所以C的预测错误），D错误（提升的是C），只有A正确，符合条件；故推理成立。
        5.给出结论：获奖者是C。
"""
    },
    {
        "题目": "一组数据为2, 4, 6, 8, 10，求这组数据的平均数和中位数。",
        "解答": "平均数=(2+4+6+8+10)/5=6，中位数是中间的数6。"
    },
    {
        "题目": "求数列a_n = 1/n的极限。",
        "解答": "当n趋向于无穷大时，1/n会越来越接近0，因此数列a_n的极限是0。"
    },
    {
        "题目": "求数列a_n = (2n + 1)/(3n - 2)的极限。",
        "解答": "将分子分母同时除以n，得到(2 + 1/n)/(3 - 2/n)，当n趋向于无穷大时，1/n和2/n都趋向于0，因此极限是2/3。"
    },
    {
        "题目": "求数列a_n = sqrt(n^2 + 1) - n的极限。",
        "解答": "将表达式乘以共轭表达式，得到(sqrt(n^2 + 1) - n)(sqrt(n^2 + 1) + n)/(sqrt(n^2 + 1) + n) = 1/(sqrt(n^2 + 1) + n)，当n趋向于无穷大时，分母趋向于无穷大，因此极限是0。"
    },
    {
        "题目": "求数列a_n = (sin n)/n的极限。",
        "解答": "因为|sin n| <= 1，所以|(sin n)/n| <= 1/n，当n趋向于无穷大时，1/n趋向于0，根据夹逼定理，极限是0。"
    },
    {
        "题目": "求数列a_n = (3^n + 2^n)/(3^n - 2^n)的极限。",
        "解答": "将分子分母同时除以3^n，得到(1 + (2/3)^n)/(1 - (2/3)^n)，当n趋向于无穷大时，(2/3)^n趋向于0，因此极限是1/1 = 1。"
    },
    {
        "题目": "求数列a_n = n^(1/n)的极限。",
        "解答": "取自然对数，得到ln(a_n) = (ln n)/n，当n趋向于无穷大时，(ln n)/n趋向于0，因此ln(a_n)趋向于0，所以a_n趋向于e^0 = 1。"
    },
    {
        "题目": "求数列a_n = (1 + 1/n)^n的极限。",
        "解答": "这是一个著名的极限，当n趋向于无穷大时，(1 + 1/n)^n趋向于数学常数e，因此极限是e。"
    },
    {
        "题目": "求数列a_n = (n^2 + 1)/(2n^2 + 3n + 1)的极限。",
        "解答": "将分子分母同时除以n^2，得到(1 + 1/n^2)/(2 + 3/n + 1/n^2)，当n趋向于无穷大时，1/n^2、3/n都趋向于0，因此极限是1/2。"
    },
    {
        "题目": "求数列a_n = (5n + 3)/(2n + 7)的极限。",
        "解答": "将分子分母同时除以n，得到(5 + 3/n)/(2 + 7/n)，当n趋向于无穷大时，3/n和7/n都趋向于0，因此极限是5/2。"
    },
    {
        "题目": "求数列a_n = (cos n)/n的极限。",
        "解答": "因为|cos n| <= 1，所以|(cos n)/n| <= 1/n，当n趋向于无穷大时，1/n趋向于0，根据夹逼定理，极限是0。"
    }
]